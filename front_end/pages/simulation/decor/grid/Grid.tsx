...
const Grid = () => {
  ...
  return (
    <section className="gamegrid">
      {mappedGamegrid && mappedGamegrid.map((v:gridcoords [])=> v.map(({gridX, gridY}:gridcoords)=><div key={`${gridX} + ${gridY}`} id={`x${gridX}y${gridY}`} className={showGrid ? "gamegrid__cell-show":"gamegrid__cell"}>{showCoords && `X:${gridX}, Y:${gridY}`}</div>))}
    </section>
  )
}
